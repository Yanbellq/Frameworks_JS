...
class LibraryApp {
    ...
    private handleAddUser(): void {
        ...
        const validation = Validator.validateUser(
            nameInput.value,
            emailInput.value
        );
        ...
        if (paginatedBooks.length === 0) {
            ...
            booksList.innerHTML = paginatedBooks.map(book => `
                <div class="border rounded-lg p-4 ${book.isBorrowed ? 'bg-yellow-50' : 'bg-white'} flex items-center justify-between">
                    <p class="font-semibold text-gray-800">${book.getInfo()}</p>
    ...
    private renderPagination(elementId: string, totalPages: number, currentPage: number, onPageChange: (page: number) => void): void {
